...
% 		min_count=3000;	% minimum counts captured in analysis to pass
        ...
        if ~outerr
            [outval,outerr]=PosAnal(filepath,startfile,files,window,false,0.61,false);
        end
        
        % Error handling for PosAnal call
        %   sometimes the script doesn't capture the right number of pulses
        if outerr || size(outval,1)~=fopts.n_pulse
            cost_total = 100;
            unc_total = 100;
            bad = 'True';
            output=[];
...
%             disp('outerr || size of outval error');
            ...
            numcounts=sum(n_capt);
            numpulses=size(t_cent,1);
			
            %check the number of counts to see if we had a good run
            if numcounts<min_count
                cost_total = 100;
                unc_total = 100;
                bad = 'True';
            else
                ...
                costx=std(x_avg);
                output.osc_std(1)=std(x_avg);   % get oscillation
                ...
                uncx = sqrt(sum((x_std./sqrt(n_capt)).^2))/numpulses;
                uncx=uncx+0.05*costx;
                
				%% Y-oscillations
                meany=mean(y_avg);
                %fourier of average y position
                ffty=FFTxt(t_cent,y_avg-meany);
                %eliminate low level frequencies
				...
				meanz=mean(dz_avg);				% average z-deviation from window centre
				
				% Fourier of avg dZ
				fftz=FFTxt(t_cent,dz_avg-meanz);
				
				%eliminate low level frequencies
                fftz_restricted = fftz(fftz(:,1)>lower_freq_bound,:);
                
                % Calculate cost factor from dZ
				costz=std(dz_avg);	% cost measure from dZ
                output.osc_std(3)=std(dz_avg);   % get oscillation
				penalty_z=penalty_pkpk(dz_avg,0.95*dw*v_tof);
                costz=costz+penalty_z;
                
				uncz = sqrt(sum((dz_std./sqrt(n_capt)).^2))/numpulses;
				...
				cost_total = sqrt(sum([costx,costy,costz].^2));
				unc_total = sqrt(sum([uncx,uncy,uncz].^2));
				bad = 'False';
